...
### 1c-2. 下拉選項庫（業務表單 ↔ 選項庫連動）

> 與 §1c「表頭／明細 AB」不同：這裡的連動是**下拉欄位引用共用選項清單**。

| 項目 | 定案／方向 |
|------|------------|
| **業務表單（A）** | 申請單欄位／流程設計；下拉欄位用 `options_ref` 指到選項庫 |
| **選項庫（B）** | 專管下拉 `items[]`；以**帳號**為單位建立／管理（本機 key：`approval.option_lists.v1`） |
| **連動** | A 欄位 `options_ref = list_id`；執行期讀 B 的 `items[]`（有讀權才解析） |
| **可手填** | 指**申請人填單時**可輸入清單外值（`allow_manual`）；**設計檔不另開文字欄** |
| **可空白** | `allow_blank`；申請人可不選 |
| **公司來源** | 在 B item 設 `readers`（帳號或 `*`）讓其他帳號可讀該清單；owner 可編 |
| **UI** | 設計列 default：可空白／可手填燈號＋`B：清單名`＋⋯ 開選項庫；設計列表有「B 下拉選項庫」 |
...
### 4.8-2. Mail 棋盤儀表板（UI 偏好）

| 項目 | 定案／方向 |
|------|------------|
| **感覺** | 儀表板／樂高：橢圓按鈕＝物件化材料 |
| **棋盤** | 背後網格；**左右＝階段**、**上下＝角色列** |
| **三種屬性** | ①**人員**（Requester／Approver／FYI／CC…）②**動作**（Submit／Approve／Reject…）＋**寄信** ③**輸入欄位**（此階段要填哪些） |
| **怎麼放** | **點材料 → 再點格子**（主操作）；也可拖拉；選取後點「丟棄」刪除 |
| **材料可新增** | ＋人員、＋寄信、＋選欄位 |
| **編輯信** | 選取寄信物件再點一次 → 編主旨／內文；版面存 `mail_board`（schema 2） |
...
| Mail 棋盤 | 三種屬性物件化：人員／動作+寄信／欄位；點選放置；mail_board schema2 |
| 下拉選項庫 | 業務表單 `options_ref` ↔ 帳號級 B 清單；`readers` 控公司可讀；可手填＝申請端 |
