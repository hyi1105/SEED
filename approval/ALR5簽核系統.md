...
| 項目 | 定案／方向 |
|------|------------|
| **感覺** | 儀表板／樂高：橢圓狀態按鈕＝可用材料 |
| **棋盤** | 背後網格；**左右＝階段**、**上下＝參與者** |
| **材料** | 狀態、旗子、箭頭、通知信皆用橢圓按鈕呈現；可拖拉到格子 |
| **旗子** | 路徑標記（結案／取消／隨時等）也用按鈕，不用純文字標籤 |
| **編輯信** | 點信封（mail）節點編主旨／內文；版面存 `approval.columns[].mail_board` |
...
| Mail 棋盤 | 左右階段、上下參與者；橢圓材料可拖；版面存 mail_board |
