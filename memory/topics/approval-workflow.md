...
- **兩層結構**：簽核範本（設定）與申請單（複製後填寫）分開，不再混在同一畫面。
- **問卷式設計**：從 System Seed → 簽核 → 空白簽核，點進去直接是欄位設計畫面（像問卷），可加入單行、多行、下拉選單；流程／權限／通知收在「進階設定」。
- 範本不再用分頁（欄位／流程／權限…）；一進來就是設計畫面。
- 從範本按「建立申請單」會複製一份獨立 SEED 供填寫。
- 申請單以問卷式直式欄位呈現，支援 Save 草稿、Submit、Approve、Deny、Return、Delegate、Call。
- **填寫視角**：預設 Requester；送出後 Requester 欄位鎖定；點流程人頭或角色列可切換 Approver 視角。
- **Return**：可指定退回 Requester 或前一階段。
- **Call**：可編輯通知對象清單（前端紀錄，未寄信）。
- 流程圖可切換「依階段／以人為主」；以人為主時同一人只出現一次，委派以回簽箭頭區塊顯示。
- 範本「申請單」頁籤列出狀態、送出／簽核日期，可刪除單筆申請。
- 「我的 SEED」簽核範本卡片可直接按 ＋ 建立申請單。
